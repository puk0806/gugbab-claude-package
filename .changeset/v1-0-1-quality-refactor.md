...
# v1.0.1 — 품질 리팩토링 (RSC 호환성 + SSR 안전성 + 캡슐화 정돈)
...
- 9 publishable 패키지의 `engines.node` 를 `>=20.17.0` → `>=18.0.0` 으로 *완화*. publishable 패키지의 `engines` 는 *소비자* 환경 호환을 의미하므로 React 라이브러리 표준값(Node 18+)을 따른다.
