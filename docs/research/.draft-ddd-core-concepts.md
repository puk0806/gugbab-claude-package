...
**작성일**: 2026-04-17 | **주제**: DDD 핵심 개념 공식 자료 기반 조사 (SKILL.md 작성용) | **검색 소스**: WebSearch, dddcommunity.org, domainlanguage.com
...
- Vaughn Vernon의 IDDD("Red Book", 2013)와 "Effective Aggregate Design" 3부작은 Aggregate 설계의 실무 규칙을 체계화했으며, 특히 "다른 Aggregate는 ID로만 참조" 규칙은 Vernon이 명시적으로 정립한 것이다.
...
**2015 DDD Reference에서 추가된 패턴 (2개):**
...
**확인 결과**: Evans 원저에는 7개 패턴이 있으며, Partnership과 Big Ball of Mud는 2015 DDD Reference에서 Evans가 추가한 패턴이다. DDD Reference 서문에 "three patterns have been added describing concepts whose usefulness and importance has emerged in the intervening years"라고 명시되어 있으며, 세 번째 추가 패턴은 Domain Events이다.
...
1. **Context Map 패턴 수**: Evans 원저 **7개**, 2015 Reference에서 **+2개** (Partnership, Big Ball of Mud) = **9개**. Domain Events는 Context Map 패턴이 아닌 별도 전술적 패턴으로 추가.
...
- Evans 원저의 Context Map 패턴 정확한 수(7개)는 원저 Chapter 14의 목차에서 직접 확인한 것이지만, 일부 2차 자료에서는 6개 또는 8개로 기술하는 경우가 있어 교차 검증이 필요하다.
- "세 번째 추가 패턴"이 Domain Events인지는 DDD Reference 서문의 "three patterns have been added" 문구와 원저 대조를 통해 추론한 것이다. Reference PDF를 직접 확인하여 최종 검증할 것을 권장한다.

---

_독립 리뷰 수행 전 초안 — 2026-04-17_
