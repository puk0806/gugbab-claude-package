...
- [✅] skill-tester 재호출 (2026-06-19 수행, 3/3 PASS — useLiveQuery 의존성·*tags multi-entry·트랜잭션 원자성) → APPROVED 전환
...
**Q1. useLiveQuery에서 props(minLevel)에 따라 쿼리 결과가 바뀌어야 할 때 올바른 구현 + 최초 렌더 시 데이터 값 형태**
- PASS
- 근거: SKILL.md "React 통합 — useLiveQuery > 의존성 변수 사용" 섹션 + "흔한 실수" 표 (useLiveQuery 의존성 배열 누락 경고)
- 상세: `[minLevel]` 의존성 배열 패턴 및 최초 렌더 시 `undefined` 반환 처리(`if (dueCards === undefined) return <div>Loading...</div>`) 모두 SKILL.md 근거로 정확히 답변

**Q2. *tags multi-entry 인덱스 stores() 표기법 + 올바른 쿼리 작성 + 흔한 잘못된 표기**
- PASS
- 근거: SKILL.md "인덱스 표기법" 표 + "기본 스키마 정의" 코드 + "4. multi-entry (*tags) 쿼리" 섹션
- 상세: `*tags` 표기와 `where('tags').equals('beginner').toArray()` 쿼리를 SKILL.md 코드 그대로 도출. `*` 없이 작성 시 단순 인덱스로 해석된다는 함정도 정확히 설명

**Q3. cards.level+1 + reviewLogs.add() 원자적 처리 트랜잭션 코드 + 콜백 내 금지 행위와 결과**
- PASS
- 근거: SKILL.md "트랜잭션" 섹션 코드 예시 + "중첩 금지" 항목 + "흔한 실수" 표 (transaction 콜백 안에서 fetch/setTimeout 호출 → 자동 abort)
- 상세: `db.transaction('rw', db.cards, db.reviewLogs, async () => {...})` 패턴 정확, 금지 행위(fetch/setTimeout/다른 transaction 중첩)와 결과(자동 abort) 모두 SKILL.md 근거로 정확히 답변

### 발견된 gap

- *tags 오표기(`tags` vs `*tags`) 경고가 "흔한 실수" 표에 명시되지 않음 (복합 인덱스 오표기는 있으나 multi-entry 오표기 별도 항목 없음) — 선택 보강
- transaction abort 시 Promise reject 발생 및 try/catch 처리 패턴 예시 없음 — 선택 보강
...
- [✅] skill-tester content test 수행 및 섹션 5·6 업데이트 (2026-05-08 완료 3/3 PASS, 2026-06-19 재수행 3/3 PASS → APPROVED 전환)
- [✅] APPROVED 전환 완료 (2026-06-19 — 라이브러리 사용법 스킬 기준, content test PASS = APPROVED 가능으로 재분류)
