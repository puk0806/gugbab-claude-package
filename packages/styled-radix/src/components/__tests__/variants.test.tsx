...
import { Accordion, AlertDialog, Combobox, Dialog, Form, Pagination, Select, Slider, Toggle, ToggleGroup } from "../..";
...
describe("Accordion variant", () => {
    it.each(["default", "outline"] as const)("renders grx-accordion--%s", (variant) => {
        ...
        expect(container.querySelector('[data-testid="root"]')).toHaveClass(`grx-accordion--${variant}`);
    });
    ...
    it('default variant is "default"', () => {
        ...
        expect(container.querySelector('[data-testid="root"]')).toHaveClass("grx-accordion--default");
    });

    it("Item has grx-accordion__item class", () => {
        const { container } = render(
...
describe("AlertDialog Action variant", () => {
    it.each(["accent", "danger"] as const)("Action renders variant %s", (variant) => {
        ...
        expect(el).toHaveClass(`grx-alert-dialog__action--${variant}`);
    });
});
...
describe("Dialog Content size", () => {
    it.each(["sm", "md", "lg", "xl"] as const)("renders content size %s", (size) => {
        ...
        expect(el).toHaveClass(`grx-dialog__content--${size}`);
    });
});

describe("Pagination size", () => {
    it.each(["sm", "md"] as const)("renders grx-pagination--%s", (size) => {
        const { container } = render(
...
describe("Slider size", () => {
    it.each(["sm", "md"] as const)("renders grx-slider--%s", (size) => {
        const { container } = render(
...
describe("Combobox Input size", () => {
    ...
    it.each(["sm", "md"] as const)("Input carries grx-combobox__input--%s", (size) => {
        const { container } = render(
            <Combobox.Root>
                <Combobox.Anchor>
        ...
        expect(input).toHaveClass(`grx-combobox__input--${size}`);
    });
});

describe("Select Trigger size", () => {
    it.each(["sm", "md"] as const)("renders grx-select__trigger--%s", (size) => {
        const { container } = render(
            <Select.Root>
        ...
        expect(container.querySelector('[data-testid="trigger"]')).toHaveClass(`grx-select__trigger--${size}`);
    });
});
...
describe("ToggleGroup variant + size", () => {
    it.each(["default", "outline"] as const)("renders variant %s", (variant) => {
        ...
        expect(container.querySelector('[data-testid="root"]')).toHaveClass(`grx-toggle-group--${variant}`);
    });
    ...
    it.each(["sm", "md"] as const)("renders size %s", (size) => {
        ...
        expect(container.querySelector('[data-testid="root"]')).toHaveClass(`grx-toggle-group--${size}`);
    });
});

describe("Form Field status", () => {
    it.each(["error", "success", "warning"] as const)("Field renders grx-form__field--%s", (status) => {
        const { container } = render(
            <Form.Root>
        ...
        expect(container.querySelector('[data-testid="field"]')).toHaveClass(`grx-form__field--${status}`);
    });
    ...
    it("Field default status does not add status modifier", () => {
        ...
        expect(el?.className).not.toMatch(/grx-form__field--error|--success|--warning/);
    });
});

describe("consumer className merge — variant components", () => {
    it("Toggle merges custom className with variant + size", () => {
        const { container } = render(<Toggle data-testid="t" variant="outline" size="sm" className="custom" />);
