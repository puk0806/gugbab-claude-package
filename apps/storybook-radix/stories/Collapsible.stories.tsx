import { Collapsible } from "@gugbab/styled-radix";
...
function Demo({ defaultOpen = false, disabled }: DemoProps) {
    return (
        <Collapsible.Root defaultOpen={defaultOpen} disabled={disabled} style={{ width: 320 }}>
            <Collapsible.Trigger style={{ width: "100%", textAlign: "left" }}>Toggle section</Collapsible.Trigger>
            <Collapsible.Content>
                <p style={{ margin: "8px 0 0" }}>
                    This content is revealed when the trigger is activated. It can contain any content you need.
                </p>
            </Collapsible.Content>
        </Collapsible.Root>
    );
...
export const Default: Story = {
    parameters: {
        docs: {
            description: {
                story: "`defaultOpen=false` 초기 상태. 트리거 클릭 또는 Space / Enter 키로 콘텐츠를 펼친다.",
            },
        },
    },
    render: () => <Demo />,
...
export const DefaultOpen: Story = {
    parameters: {
        docs: {
            description: {
                story: "`defaultOpen=true` — 처음부터 콘텐츠가 펼쳐진 상태로 렌더된다. 초기 노출이 필요한 FAQ 섹션에 적합.",
            },
        },
    },
...
export const Disabled: Story = {
    parameters: {
        docs: {
            description: {
                story: "`disabled=true` — 트리거가 비활성화되어 열림 상태가 고정된다. 접근성상 `aria-disabled` 가 함께 반영된다.",
            },
        },
    },
