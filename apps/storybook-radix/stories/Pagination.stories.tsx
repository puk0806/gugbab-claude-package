import { Pagination, type PaginationSize } from "@gugbab/styled-radix";
...
function EllipsisDemo({ size, pageCount = 20, defaultPage = 10 }: DemoProps) {
    ...
    const items: Array<number | null> = [1, null, page - 1, page, page + 1, null, pageCount].filter((v, i, arr) => {
...
export const Default: Story = {
    parameters: {
        docs: {
            description: {
                story: "7페이지 기본 구성. 현재 페이지(3)가 활성 상태이며 Previous/Next 버튼으로 이동할 수 있다.",
            },
        },
    },
    render: () => <Demo />,
...
export const ManyPages: Story = {
    parameters: {
        docs: {
            description: {
                story: "20페이지처럼 페이지 수가 많을 때 `Ellipsis`로 중간 범위를 생략하는 패턴. 현재 페이지 주변만 노출한다.",
            },
        },
    },
