    ...
    return (
        <Toast.Provider duration={4000}>
            ...
            <Toast.Root open={open} onOpenChange={setOpen}>
                ...
                <Toast.Description>Your meeting has been scheduled for tomorrow at 10 AM.</Toast.Description>
...
export const Default: Story = {
    parameters: {
        docs: {
            description: {
                story: "버튼 클릭 시 토스트가 나타나고 4초 후 자동으로 사라진다. 닫기 버튼으로 즉시 해제도 가능하다.",
            },
        },
    },
    render: () => <SimpleDemo />,
...
export const WithAction: Story = {
    parameters: {
        docs: {
            description: {
                story: "`Toast.Action`을 추가한 케이스. Undo처럼 직전 동작을 되돌릴 수 있는 버튼을 토스트 내부에 배치한다. `altText`는 스크린 리더 전용 대체 텍스트로, 키보드·스크린 리더 사용자에게 동일한 동작에 도달할 다른 경로를 안내한다.",
            },
        },
    },
...
export const MultipleToasts: Story = {
    parameters: {
        docs: {
            description: {
                story: "버튼 클릭마다 토스트를 큐에 추가. `Viewport`가 스택을 관리하며 닫히면 목록에서 제거된다.",
            },
        },
    },
    render: () => {
        ...
        const addToast = () => {
            setToasts((prev) => [...prev, { id: Date.now(), message: `Notification #${prev.length + 1}` }]);
        };
