import { RadioGroup, type RadioGroupSize } from "@gugbab/styled-mui";
...
function Demo({ size, defaultValue = "a", disabled }: DemoProps) {
    return (
        <RadioGroup.Root size={size} defaultValue={defaultValue} disabled={disabled} aria-label="example">
...
export const Default: Story = {
    parameters: {
        docs: {
            description: {
                story: '`defaultValue="a"`로 첫 번째 항목이 미리 선택된 기본 상태. Arrow 키로 항목 간 이동이 가능하다.',
            },
        },
    },
    render: () => <Demo />,
...
export const Sizes: Story = {
    parameters: {
        docs: {
            description: {
                story: "`size` prop으로 `sm` / `md` 두 단계 크기 비교. 버튼과 인디케이터 크기가 함께 조정된다.",
            },
        },
    },
    render: () => (
...
export const Disabled: Story = {
    parameters: {
        docs: {
            description: {
                story: "`Root`에 `disabled`를 주면 그룹 전체가 비활성화된다. 선택 값은 유지되지만 변경 불가.",
            },
        },
    },
